...
# The code now imports performance optimizers and uses them to enhance speed by optimizing caching, data loading, and reducing API calls.
...
def generate_consensus_analysis(selected_game, game_idx: int):
    """Generate consensus analysis for selected game"""
    date_str = st.session_state.get('selected_date', date.today().strftime('%Y-%m-%d'))

    # Check if we should update data
    if not stream_optimizer.should_update('predictions'):
        cache_key = f"consensus_analysis_{game_idx}_{date_str}"
    ...
    if cache_key not in st.session_state:
        with st.spinner("Generating dual AI consensus..."):
            try:
                consensus = st.session_state.consensus_engine.analyze_game_consensus(selected_game.to_dict())
