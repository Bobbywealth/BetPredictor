...
def get_daily_bets(date=None):
    """Get daily bets from database - only picks actually generated by this user"""
    if date is None:
        date = datetime.now().date()
    
    date_str = date.isoformat()
    
    # Try database first - filter by user and actual generation date
    supabase = init_supabase()
    if supabase:
        try:
            user_id = get_or_create_user_id()
            ...
            result = supabase.table('predictions')\
                ...
                .lt('created_at', f"{(date + timedelta(days=1)).isoformat()}T00:00:00")\
                .order('bet_rank')\
                .execute()
            ...
            with col2:
                ...
                if button_clicked:
                    if len(final_games) > 0:
                        ...
                        st.error("No games available to score")
...
def show_daily_betting_tracker():
    ...
    if not daily_bets:
        ...
        # Debug info to help user understand
        user_id = get_or_create_user_id()
        if st.session_state.get('debug_mode', False):
            st.write(f"🔍 Debug: Looking for picks with user_id='{user_id}' and game_date='{selected_date.isoformat()}'")
